package br.com.btsoftware.algamoney.api.storage;
...
public class S3 {
	...
	public String salveTemporary(MultipartFile file) {
		...
		try {
			PutObjectRequest putObjectRequest = new PutObjectRequest(property.getS3().getBucket(), uniqueName,
					file.getInputStream(), objectMetadata).withAccessControlList(acl);
	...
	public void replace(String oldObj, String newObj) {
		if (StringUtils.hasText(oldObj)) {
			this.remove(oldObj);
		}
